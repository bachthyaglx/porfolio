  ...
  return (
    <nav className="bg-black text-white px-6 py-4 flex items-center justify-between shadow-md sticky top-0 z-50">
      ...
      <div>
        {!isLoggedIn ? (
          ...
          <button
            ...
            className="transition ease-in-out duration-300 text-center border-2 border-red-500 text-white-500 rounded-3xl px-3 py-1 hover:bg-red-500 hover:text-white"
          >
            Logout
          </button>
        )}
      ...
      <LoginModal
        ...
        onClose={() => {
          setShowLogin(false);
          setIsLoggedIn(true); // login thành công
        }}
      />
    </nav>
  );
